...
export default function AdminLogin({ onLogin }: Props) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setLoading(true);
    setErr("");
    const result = await adminSignIn(email, pass);
    setLoading(false);
    if (result.success) {
      onLogin();
    } else {
    ...
    <div className="min-h-screen flex items-center justify-center relative overflow-hidden"
      ...
      <motion.div
        ...
        style={{
          ...
          boxShadow: "0 24px 80px rgba(0,0,0,0.6)", backdropFilter: "blur(24px)",
        }}>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <p className="text-[10px] text-center" style={{ color: "rgba(255,255,255,0.25)" }}>
            Demo credentials: admin@winggo.in / winggo@2024
          </p>
          ...
          <motion.button type="submit" whileTap={{ scale: 0.97 }} disabled={loading}
            ...
            style={{ background: "linear-gradient(135deg, #FFD700, #ff8c00)", color: "#000",
              boxShadow: "0 0 24px rgba(255,215,0,0.35)", opacity: loading ? 0.7 : 1 }}>
