...
export interface WorldWarRoom {
  roomId: string;
  entryFee: number;
  prizePool: number;
  teams: {
    karan: Record<string, MatchPlayer & { score: number }>;
    arjun: Record<string, MatchPlayer & { score: number }>;
  };
...
export async function joinWorldWarRoom(
  entryFee: number,
  player: MatchPlayer,
  team: "karan" | "arjun"
): Promise<{ roomId: string }> {
  if (!FIREBASE_ENABLED || !rtdb) {
    return { roomId: `ww-demo-${Date.now()}` };
  }

  const wwRef = ref(rtdb, `worldwar/${entryFee}`);
  const snap = await get(wwRef);
  const rooms = snap.val() as Record<string, WorldWarRoom> | null;

  if (rooms) {
    for (const [roomId, room] of Object.entries(rooms)) {
      if (room.status === "waiting") {
        await update(ref(rtdb, `worldwar/${entryFee}/${roomId}/teams/${team}/${player.uid}`), {
          ...player, score: 0,
        });
        return { roomId };
      }
  ...
  const newRef = push(wwRef);
  const roomId = newRef.key!;
  const room: WorldWarRoom = {
    roomId,
    entryFee,
    prizePool: entryFee * 18 * 0.9,
    teams: {
      karan: team === "karan" ? { [player.uid]: { ...player, score: 0 } } : {},
      arjun: team === "arjun" ? { [player.uid]: { ...player, score: 0 } } : {},
    },
    scores: { karan: 0, arjun: 0 },
    status: "waiting",
    createdAt: Date.now(),
    timer: 180,
  };
...
export function subscribeWorldWarRoom(
  entryFee: number,
  roomId: string,
  cb: (room: WorldWarRoom | null) => void
): () => void {
  if (!FIREBASE_ENABLED || !rtdb) return () => {};
  const r = ref(rtdb, `worldwar/${entryFee}/${roomId}`);
  const handler = (snap: { val: () => WorldWarRoom | null }) => cb(snap.val());
  onValue(r, handler as Parameters<typeof onValue>[1]);
  return () => off(r, "value", handler as Parameters<typeof onValue>[1]);
}

export async function updateWorldWarScore(
  entryFee: number,
  roomId: string,
  team: "karan" | "arjun",
  uid: string,
  ...
  if (!FIREBASE_ENABLED || !rtdb) return;
  const roomRef = ref(rtdb, `worldwar/${entryFee}/${roomId}`);
  const snap = await get(roomRef);
  if (!snap.exists()) return;
  const room = snap.val() as WorldWarRoom;
  const newTeamScore = (room.scores[team] ?? 0) + scoreDelta;
  const playerScore  = ((room.teams[team][uid]?.score) ?? 0) + scoreDelta;
  await update(roomRef, {
    [`teams/${team}/${uid}/score`]: playerScore,
    [`scores/${team}`]: newTeamScore,
  });
}
