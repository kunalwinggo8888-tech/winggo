  ...
  return (
    ...
    >
      ...
      <main className="flex-1 overflow-y-auto pb-24 pt-16">
        ...
        {/* ─── WORLD WAR PREMIUM BANNER ─── */}
        <motion.div
          className="mx-4 mt-4 rounded-3xl overflow-hidden relative cursor-pointer"
          style={{ background: "linear-gradient(145deg,#0f0002 0%,#18001a 40%,#00001f 100%)", border: "1.5px solid rgba(231,76,60,0.35)", boxShadow: "0 0 36px rgba(231,76,60,0.18), 0 0 36px rgba(59,130,246,0.12)" }}
          animate={{ boxShadow: ["0 0 28px rgba(231,76,60,0.15),0 0 28px rgba(59,130,246,0.1)","0 0 52px rgba(231,76,60,0.38),0 0 52px rgba(59,130,246,0.25)","0 0 28px rgba(231,76,60,0.15),0 0 28px rgba(59,130,246,0.1)"] }}
        ...
        >
          ...
          <div className="absolute inset-0 pointer-events-none overflow-hidden">
            <motion.div className="absolute w-48 h-48 rounded-full"
              style={{ background: "radial-gradient(circle,rgba(231,76,60,0.32) 0%,transparent 65%)", top:"-30%",left:"-5%" }}
              animate={{ scale:[1,1.25,1] }} transition={{ duration:3.5, repeat:Infinity }} />
            <motion.div className="absolute w-48 h-48 rounded-full"
              style={{ background: "radial-gradient(circle,rgba(59,130,246,0.32) 0%,transparent 65%)", top:"-30%",right:"-5%" }}
              animate={{ scale:[1,1.2,1] }} transition={{ duration:3, repeat:Infinity, delay:1 }} />
            {/* Lightning streaks */}
            {[...Array(4)].map((_,i) => (
              <motion.div key={i} className="absolute"
                style={{ width:1.5, height:i%2===0?50:70, left:`${20+i*18}%`, top:`${10+i*8}%`,
                  background:"linear-gradient(180deg,rgba(59,130,246,0.85),transparent)", transformOrigin:"top" }}
                animate={{ opacity:[0,1,0], scaleX:[1,2+i,1] }}
                transition={{ duration:0.14+i*0.06, delay:i*0.55+0.3, repeat:Infinity, repeatDelay:3+i*0.8 }} />
            ))}
            {/* Fire sparks */}
            {[...Array(16)].map((_,i) => (
              <motion.div key={`s${i}`} className="absolute rounded-full"
                style={{ width:i%3===0?4:3, height:i%3===0?4:3, left:`${8+i*5.5}%`, bottom:`${4+Math.sin(i)*12}%`,
                  background: i%2===0?"#ff4500":"#FFD700", boxShadow:i%2===0?"0 0 5px #ff4500":"0 0 5px #FFD700" }}
                animate={{ y:[0,-(25+i*3)], opacity:[1,0] }}
                transition={{ duration:0.7+Math.random()*0.9, delay:i*0.18, repeat:Infinity }} />
            ))}
          </div>
          ...
          <div className="relative z-10 px-5 py-4">
            ...
            <div className="flex items-center justify-between mb-3">
              <div className="flex items-center gap-2">
                <motion.div className="w-2 h-2 rounded-full bg-red-500"
                  animate={{ opacity:[1,0.2,1], scale:[1,1.4,1] }} transition={{ duration:0.8, repeat:Infinity }} />
                <span className="text-[10px] font-black tracking-wider" style={{ color:"rgba(255,255,255,0.4)" }}>WINGGO EXCLUSIVE</span>
              </div>
            ...
            {/* Main heading */}
            <motion.div className="text-3xl font-black text-white leading-none mb-1"
              style={{ textShadow:"0 0 28px rgba(255,215,0,0.7)" }}
              animate={{ textShadow:["0 0 18px rgba(255,215,0,0.4)","0 0 45px rgba(255,215,0,1)","0 0 18px rgba(255,215,0,0.4)"] }}
              transition={{ duration:2, repeat:Infinity }}>
              ⚔️ WORLD WAR
            </motion.div>
            <p className="text-[11px] font-bold tracking-widest mb-4" style={{ color:"rgba(255,255,255,0.3)" }}>BATTLE FOR GLORY</p>

            {/* Teams */}
            <div className="flex items-center gap-4 mb-4">
              {/* Karna */}
              <motion.div className="flex flex-col items-center gap-1" animate={{ x:[0,-3,0] }} transition={{ duration:2, repeat:Infinity }}>
                <div className="w-14 h-14 rounded-2xl flex items-center justify-center text-3xl relative"
                  style={{ background:"rgba(231,76,60,0.2)", border:"2px solid rgba(231,76,60,0.6)", boxShadow:"0 0 20px rgba(231,76,60,0.5)" }}>
                  ⚔️
                  <motion.div className="absolute -bottom-1 -right-1 text-xs" animate={{ scale:[1,1.5,1] }} transition={{ duration:0.6, repeat:Infinity }}>🔥</motion.div>
                </div>
                <span className="text-[10px] font-black" style={{ color:"#f87171" }}>🔥 KARNA</span>
              </motion.div>

              {/* VS + stats */}
              <div className="flex-1">
                <div className="flex items-center justify-center gap-2 mb-2">
                  <motion.div className="text-xl font-black text-white" style={{ textShadow:"0 0 12px rgba(255,215,0,0.9)" }}
                    animate={{ scale:[1,1.25,1] }} transition={{ duration:0.75, repeat:Infinity }}>VS</motion.div>
                  <motion.div className="text-lg" animate={{ rotate:[0,20,-20,0] }} transition={{ duration:0.6, repeat:Infinity }}>⚡</motion.div>
                </div>
                {/* Live score preview bar */}
                <div className="flex h-2.5 rounded-full overflow-hidden" style={{ background:"rgba(255,255,255,0.07)" }}>
                  <motion.div className="h-full" style={{ background:"linear-gradient(90deg,#e74c3c,#ff6b35)", width:"52%" }}
                    animate={{ width:["48%","55%","51%"] }} transition={{ duration:3, repeat:Infinity }} />
                  <div className="h-full flex-1" style={{ background:"linear-gradient(90deg,#3b82f6,#60a5fa)" }} />
                </div>
                <div className="flex justify-between text-[8px] font-black mt-0.5">
                  <span style={{ color:"#f87171" }}>52%</span>
                  <span style={{ color:"rgba(255,255,255,0.2)" }}>LIVE</span>
                  <span style={{ color:"#60a5fa" }}>48%</span>
                </div>
              </div>
              ...
              <motion.div className="flex flex-col items-center gap-1" animate={{ x:[0,3,0] }} transition={{ duration:2, repeat:Infinity }}>
                <div className="w-14 h-14 rounded-2xl flex items-center justify-center text-3xl relative"
                  style={{ background:"rgba(59,130,246,0.2)", border:"2px solid rgba(59,130,246,0.6)", boxShadow:"0 0 20px rgba(59,130,246,0.5)" }}>
                  🛡️
                  <motion.div className="absolute -bottom-1 -right-1 text-xs" animate={{ scale:[1,1.5,1] }} transition={{ duration:0.6, repeat:Infinity, delay:0.3 }}>⚡</motion.div>
                </div>
                <span className="text-[10px] font-black" style={{ color:"#60a5fa" }}>⚡ ARJUN</span>
              </motion.div>
            </div>
            ...
            <div className="flex items-center gap-2 mb-4">
              {[
                { icon:"👥", label:"14.8K Warriors" },
                { icon:"🔴", label:"LIVE NOW" },
                { icon:"⚡", label:"Entry ₹2 onwards" },
              ].map(({ icon, label }) => (
                <div key={label} className="flex items-center gap-1 px-2 py-1 rounded-full"
                  style={{ background:"rgba(255,255,255,0.06)", border:"1px solid rgba(255,255,255,0.09)" }}>
                  <span className="text-[10px]">{icon}</span>
                  <span className="text-[9px] font-bold" style={{ color:"rgba(255,255,255,0.5)" }}>{label}</span>
                </div>
              ))}
            </div>

            {/* CTA */}
            <motion.div className="w-full py-3 rounded-2xl text-center font-black text-base relative overflow-hidden"
              style={{ background:"linear-gradient(135deg,#e74c3c,#3b82f6)", color:"#fff", boxShadow:"0 0 22px rgba(231,76,60,0.45)", letterSpacing:"0.04em" }}>
              <motion.div className="absolute inset-0 pointer-events-none"
                style={{ background:"linear-gradient(105deg,transparent 40%,rgba(255,255,255,0.18) 50%,transparent 60%)" }}
                animate={{ x:["-100%","200%"] }} transition={{ duration:1.8, repeat:Infinity, repeatDelay:0.8 }} />
              🔥 JOIN THE WAR
            </motion.div>
          </div>
        </motion.div>
