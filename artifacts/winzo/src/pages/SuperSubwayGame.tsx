...
const FREEZE_TIME   = 3.0; // 3-2-1 countdown after a crash
...
const BOT_NAMES = ["Ravi Surfer", "Neha Metro", "Arjun Dash", "Pooja Run", "Karan Racer", "Simran Skater"];
...
interface GS {
  ...
  crashed: boolean; freezeT: number;
...
// ── Component ─────────────────────────────────────────────────────────────────
export default function SuperSubwayGame({ onBack, initialFee }: Props) {
  const { addWinning } = useWallet();
  const { addMatch }   = useMatchHistory();
  const isFreeMode     = initialFee === 0;
  const prize          = Math.floor(initialFee * 2 * 0.9);
  ...
  const [ui, setUi] = useState({
    ...
    magnet: false, jetpack: false, crashed: false, countdown: 0,
  });
  ...
  function mkGS(): GS {
    return {
      ...
      botScore: 0, frame: 0, lastTs: 0, nextId: 0,
    };
  }
  ...
  // ── Draw environment (3D rail perspective) ────────────────────────────────
  function drawEnv(ctx: CanvasRenderingContext2D, frame: number) {
    const sky = ctx.createLinearGradient(0, 0, 0, HORIZON_Y);
    sky.addColorStop(0, "#1565c0"); sky.addColorStop(0.5, "#42a5f5"); sky.addColorStop(1, "#90caf9");
    ...
    for (let i = 0; i < 16; i++) {
      ...
      ctx.fillStyle = `rgba(0,0,0,${0.12 + t * 0.10})`; ctx.fillRect(lx, y + 2 * s, rx - lx, 5 * s);
  ...
  function drawGO(ctx: CanvasRenderingContext2D, go: GO, frame: number, magnet: boolean) {
    ...
    if (kind === "coin") {
      ...
      ctx.fillStyle = "#f59e0b"; ctx.beginPath(); ctx.arc(0, 0, r, 0, Math.PI * 2); ctx.fill();
      ctx.fillStyle = "#FFD700"; ctx.beginPath(); ctx.arc(0, 0, r * 0.82, 0, Math.PI * 2); ctx.fill();
      ctx.fillStyle = "rgba(255,255,220,0.7)"; ctx.beginPath(); ctx.arc(-r * 0.28, -r * 0.28, Math.max(0.3, r * 0.32), 0, Math.PI * 2); ctx.fill();
      ...
      if (sub === "train") {
        ...
        ctx.fillStyle = "#f97316"; ctx.fillRect(-w / 2, -h * 0.55, w, h * 0.08);
  ...
  function drawPlayer(ctx: CanvasRenderingContext2D, gs: GS) {
    ...
    if (pstate === "slide") {
      ...
      ctx.fillStyle = "rgba(255,255,255,0.2)"; ctx.beginPath(); ctx.ellipse(-3, -10, 8, 4, -0.3, 0, Math.PI * 2); ctx.fill();
      ctx.fillStyle = "#1d4ed8"; ctx.fillRect(-18, -4, 10, 7); ctx.fillRect(8, -4, 10, 7);
      ctx.fillStyle = "#f1f5f9"; ctx.fillRect(-20, 2, 9, 5); ctx.fillRect(11, 2, 9, 5);
      ctx.shadowBlur = 0; ctx.fillStyle = "#fcd34d"; ctx.beginPath(); ctx.arc(12, -9, 8, 0, Math.PI * 2); ctx.fill();
      ...
      ctx.save(); ctx.rotate((-swing * Math.PI) / 180); ctx.beginPath(); ctx.arc(15,  -12, 4, 0, Math.PI * 2); ctx.fill(); ctx.restore();
      ctx.shadowBlur = 0; ctx.fillStyle = "#fcd5b4"; ctx.beginPath(); ctx.arc(0, -37, 11, 0, Math.PI * 2); ctx.fill();
      ...
      ctx.fillStyle = "#f59e0b"; ctx.fillRect(-9, -40, 18, 3);
  ...
  // ── Draw HUD (You vs Opponent leaderboard) ────────────────────────────────
  function drawHUD(ctx: CanvasRenderingContext2D, gs: GS) {
    const { score, coins, timeLeft, botScore, magnet, jetpack } = gs;

    const barGrad = ctx.createLinearGradient(0, 0, 0, 92);
    barGrad.addColorStop(0, "rgba(0,0,0,0.85)"); barGrad.addColorStop(1, "rgba(0,0,0,0.55)");
    ctx.fillStyle = barGrad; ctx.fillRect(0, 0, CW, 92);
    const bl = ctx.createLinearGradient(0, 91, CW, 91);
    bl.addColorStop(0, "rgba(251,191,36,0)"); bl.addColorStop(0.5, "rgba(251,191,36,0.55)"); bl.addColorStop(1, "rgba(251,191,36,0)");
    ctx.fillStyle = bl; ctx.fillRect(0, 91, CW, 1.5);
    ...
    ctx.fillStyle = tc; ctx.font = "bold 24px system-ui,sans-serif"; ctx.textAlign = "center";
    ctx.fillText(fmtTime(timeLeft), CW / 2, 32); ctx.shadowBlur = 0;
    ctx.fillStyle = urgent ? "rgba(239,68,68,0.55)" : "rgba(255,255,255,0.28)";
    ctx.font = "7.5px system-ui,sans-serif"; ctx.fillText("TIME LEFT", CW / 2, 46);

    // Player score (right)
    ctx.textAlign = "right";
    ctx.shadowColor = "rgba(251,191,36,0.5)"; ctx.shadowBlur = 6;
    ctx.fillStyle = "#fbbf24"; ctx.font = "bold 18px system-ui,sans-serif"; ctx.fillText(score.toLocaleString(), CW - 12, 30);
    ctx.shadowBlur = 0;
    ctx.fillStyle = "rgba(255,255,255,0.28)"; ctx.font = "7.5px system-ui,sans-serif"; ctx.fillText("YOUR SCORE", CW - 12, 44);
    ctx.fillStyle = "#FFD700"; ctx.font = "bold 11px system-ui,sans-serif"; ctx.fillText(`🪙 ${coins}`, CW - 12, 60);

    // You vs Opponent leaderboard (left)
    ctx.textAlign = "left";
    ctx.fillStyle = "rgba(255,255,255,0.38)"; ctx.font = "7.5px system-ui,sans-serif"; ctx.fillText("YOU  vs  " + botName.toUpperCase(), 10, 13);
    const bw = 92; const mx = Math.max(score, botScore, 1);
    ...
    ctx.fillStyle = "#86efac"; ctx.font = "bold 7.5px system-ui,sans-serif"; ctx.fillText(`🏃 ${score}`, 10, 54);
    ctx.fillStyle = "#fca5a5"; ctx.fillText(`🤖 ${Math.floor(botScore)}`, 10, 66);
  ...
  // ── Settle match (idempotent — runs exactly once) ─────────────────────────
  const settle = useCallback((gs: GS) => {
    if (settledRef.current) return;
    settledRef.current = true;
    const won = gs.score > gs.botScore;
    if (!isFreeMode && won) addWinning(prize, `🏃 Super Subway Surfers — Won ₹${prize}`);
    addMatch({
      gameId: "supersubway",
      gameName: isFreeMode ? "Super Subway Surfers (Practice)" : "Super Subway Surfers",
      gameIcon: "🏃",
      result: won ? "win" : "loss",
      entryFee: initialFee,
      prize: !isFreeMode && won ? prize : 0,
      userScore: gs.score,
      opponentScore: Math.floor(gs.botScore),
  ...
  loopRef.current = (ts: number) => {
    ...
    if (gs.over) {
      ...
      // Character frozen — 3-2-1 countdown, then resume from the same spot
      gs.freezeT -= dt;
      if (gs.freezeT <= 0) {
        gs.crashed = false;
        gs.running = true;
      ...
      if (!gs.over) {
        ...
        for (const o of gs.objects) {
          ...
          if (o.kind === "coin") {
            ...
            // CRASH — no game over: character stops, 3-2-1 countdown, then resume
            o.gone = true;
            gs.crashed = true;
            gs.freezeT = FREEZE_TIME;
            gs.running = false;
            break;
          }
        }
    ...
    if (!gs.over) {
      ...
      setUi({
        score: gs.score, coins: gs.coins, timeLeft: 0,
  ...
  const startGame = useCallback(() => {
    ...
    setUi({ score: 0, coins: 0, timeLeft: GAME_DURATION, botScore: 0,
      over: false, started: true, magnet: false, jetpack: false, crashed: false, countdown: 0 });
    rafRef.current = requestAnimationFrame(ts => loopRef.current(ts));
  }, []);

  // ── Swipe / keyboard input ────────────────────────────────────────────────
  function doSwipe(dx: number, dy: number) {
    const gs = gsRef.current; if (!gs?.running || gs.crashed) return;
  ...
  return (
    <div style={{ background: "#0a0a0f", width: "100%", maxWidth: 480, margin: "0 auto", minHeight: "100dvh", display: "flex", flexDirection: "column" }}>
      ...
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "10px 16px", background: "linear-gradient(90deg,#b91c1c,#dc2626)", borderBottom: "2px solid #fbbf24", flexShrink: 0 }}>
        ...
        <span style={{ color: "#fff", fontWeight: 900, fontSize: 13, letterSpacing: "0.14em", textShadow: "0 0 12px rgba(251,191,36,0.7)" }}>🏃 SUPER SUBWAY SURFERS</span>
      ...
      <div style={{ flex: 1, position: "relative", display: "flex", alignItems: "flex-start", justifyContent: "center", overflow: "hidden" }}>
        ...
        <AnimatePresence>
          {ui.crashed && (
            <motion.div key="crash"
              initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
              style={{ position: "absolute", inset: 0, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", background: "rgba(80,0,0,0.45)", backdropFilter: "blur(4px)", pointerEvents: "none" }}>
              <div style={{ fontSize: 14, fontWeight: 900, color: "#fecaca", letterSpacing: "0.1em" }}>💥 CRASH! RESUMING</div>
              <motion.div key={ui.countdown}
                initial={{ scale: 2.2, opacity: 0 }} animate={{ scale: 1, opacity: 1 }}
                transition={{ duration: 0.7, ease: "easeOut" }}
                style={{ fontSize: 96, fontWeight: 900, color: "#FFD700", textShadow: "0 0 30px rgba(255,215,0,0.8)" }}>
                {ui.countdown}
              </motion.div>
            </motion.div>
          )}
        ...
        <AnimatePresence>
          {!ui.started && (
            <motion.div key="start"
              ...
              <h2 style={{ color: "#FFD700", fontWeight: 900, fontSize: 26, margin: "0 0 4px" }}>SUPER SUBWAY SURFERS</h2>
              <p style={{ color: "rgba(255,255,255,0.4)", fontSize: 12, margin: "0 0 8px" }}>3-min race against {botName} — highest score wins!</p>
              <p style={{ color: "rgba(255,215,0,0.6)", fontSize: 11, margin: "0 0 18px", fontWeight: 700 }}>Crash? No problem — you resume after 3-2-1!</p>
          ...
          {ui.over && (
            <motion.div key="over"
              ...
              <motion.div
                ...
                style={{ width: 300, borderRadius: 24, overflow: "hidden", background: "rgba(12,8,28,0.97)", border: "1px solid rgba(255,215,0,0.18)" }}>
                <div style={{ padding: "18px 0 14px", textAlign: "center", background: won ? "linear-gradient(135deg,#065f46,#10b981)" : "linear-gradient(135deg,#7f1d1d,#ef4444)" }}>
                  <div style={{ fontSize: 46, marginBottom: 4 }}>{won ? "🏆" : "💥"}</div>
                  <div style={{ color: "#fff", fontWeight: 900, fontSize: 22 }}>{won ? "YOU WIN!" : "BOT WINS!"}</div>
                  <div style={{ color: "rgba(255,255,255,0.6)", fontSize: 11, marginTop: 3 }}>
                    {won ? `You outscored ${botName}!` : `${botName} took the lead.`}
                  </div>
                </div>
                <div style={{ padding: "14px 20px", display: "flex", flexDirection: "column", gap: 9 }}>
                  {([["🏅 Your Score", ui.score.toLocaleString()], ["🪙 Coins", ui.coins], ["📏 Distance", `${distance} m`], ["🤖 Bot Score", ui.botScore.toLocaleString()]] as const).map(([label, val]) => (
                ...
                {!isFreeMode && (
                  <div style={{ margin: "0 20px 12px", padding: "8px", borderRadius: 12, textAlign: "center", fontSize: 12, fontWeight: 700, background: won ? "rgba(16,185,129,0.12)" : "rgba(239,68,68,0.09)", color: won ? "#10b981" : "#ef4444", border: `1px solid ${won ? "rgba(16,185,129,0.25)" : "rgba(239,68,68,0.2)"}` }}>
                    {won ? `+₹${prize} credited to wallet!` : `-₹${initialFee} entry fee`}
                  </div>
                )}
